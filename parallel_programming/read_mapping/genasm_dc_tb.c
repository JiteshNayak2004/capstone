...
//usual bitap implementation can have max of say a query of 32 or 64 characters not in this case can 
//be multiple words the below function handles any size query

//takes every char in pattern and creates corresponding bitmasks
unsigned long long *generatePatternBitmasks(char* pattern, int m)
{
    // count is needed as query can be more than 64 characters say 130 in which case we would need 3 unsigned long long vars
    ...
    //ullong_max is max value that can be stored so binary form will have 111s
    ...
    for (int i=0; i < m; i++)
    {
        // the below code finds index from right like in binary 
        // example ABCD A is index 0 in normal case but in reverse it is in index 3 from right
       index = count - ((m-i-1) / 64) - 1;
        
        //basically sets the bit to 0 in the pattern_mask 
        //for eg for query ABCD mask is 0111
...
//similar to above function but exclusively for ACGT
...
{
    genasm_aligner("CGTGA", "CGTA", 3, 0, 0, 0, 0);
}
